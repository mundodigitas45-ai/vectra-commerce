-- Central de Criativos IA - regeneracao dirigida por feedback humano
-- Solicitar alteracao registra a decisao e cria um job apenas para o canal afetado.
  ...
  if p_decision = 'changes_requested' then
    select job.id
     ...
     where job.company_id = p_company_id
       ...
       and job.channel_id = v_asset.channel_id
       and job.job_type = 'campaign_orchestration'
    ...
    if v_job_id is null then
      ...
      values (
        ...
        jsonb_build_object(
          'requested_outputs',
            jsonb_build_array('strategy', 'copy'),
          'revision_request',
            jsonb_build_object(
              'channel_id', v_asset.channel_id,
              'asset_id', v_asset.id,
              'asset_type', v_asset.asset_type,
              'source_version', v_asset.version,
              'source_content', v_asset.content,
              'feedback', btrim(p_feedback),
              'requested_by', p_decided_by,
              'requested_at', now()
            )
        ),
        '{}'::jsonb,
