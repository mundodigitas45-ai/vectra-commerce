...
const STORE_ID = "f4a134bb-00fc-4314-bcd5-9d5cd45f036d";
...
function mediaLimit(contentType: string) {
  return contentType.startsWith("video/") ? MAX_VIDEO_BYTES : MAX_IMAGE_BYTES;
}

function humanLimit(contentType: string) {
  return contentType.startsWith("video/") ? "150 MB" : "20 MB";
}
...
export class ProductService {
  async list() {
    ...
    const imageByProductId = new Map((productMedia ?? []).map((item) => [item.id, item.image_url]));
  ...
  async importGoogleDriveMedia(input: ImportGoogleDriveMediaInput) {
    ...
    if (!driveResponse.ok) {
      ...
      throw new Error(`Não foi possível baixar a mídia do Google Drive (${driveResponse.status}). ${body}`.trim());
    }

    const rawContentType = driveResponse.headers.get("content-type") ?? input.mime_type;
    const contentType = rawContentType.split(";")[0].trim().toLowerCase();

    if (!ALLOWED_MEDIA_MIME_TYPES.includes(contentType)) {
      throw new Error("O arquivo selecionado não é uma imagem ou vídeo compatível.");
    }

    const maxBytes = mediaLimit(contentType);
    const contentLength = Number(driveResponse.headers.get("content-length") ?? 0);
    if (contentLength > maxBytes) {
      throw new Error(`O arquivo é maior que o limite de ${humanLimit(contentType)}.`);
    }
    ...
    const { error: bucketLookupError } = await supabase.storage.getBucket(PRODUCT_MEDIA_BUCKET);
    if (bucketLookupError) {
      const { error: createBucketError } = await supabase.storage.createBucket(PRODUCT_MEDIA_BUCKET, {
        public: true,
  ...
  async create(input: CreateProductInput) {
    ...
    if (productError || !product) {
      throw productError ?? new Error("Não foi possível cadastrar o produto.");
    }
    ...
    const { data: inventory, error: inventoryError } = await supabase
      ...
      .insert({
        ...
        reserved_quantity: 0,
        minimum_quantity: input.low_stock_threshold ?? 3,
    ...
    if (inventoryError || !inventory) {
      ...
      throw inventoryError ?? new Error("Não foi possível criar o estoque do produto.");
    }
