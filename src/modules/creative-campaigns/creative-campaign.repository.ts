    ...
    const [assetsResult, approvalsResult, jobsResult] =
      await Promise.all([
        supabase
          ...
          .select(
            "id, campaign_id, channel_id, job_id, asset_type, status, version, title, content, metadata, storage_bucket, storage_path, public_url, mime_type, width, height, duration_seconds, prompt, format_key, approved_by, approved_at, created_at, updated_at"
          )
          .eq("company_id", companyId)
